...
void Game::playGame(bool isAIModeIn, ifstream& gameFile) {
    ...
    string detail;
    while (gameFile >> detail) {
        Person person(detail);
        int tick = person.getTurn();
        
        while (building.getTime() < tick) {
    ...
    while ((satisfactionIndex.getSatisfaction() > 0) || (building.getTime() < MAX_TURNS)) {
        building.prettyPrintBuilding(cout);
        satisfactionIndex.printSatisfaction(cout, false);
        checkForGameEnd();
...
bool Game::isValidPickupList(const string& pickupList, const int pickupFloorNum) const {
    Floor floor = building.getFloorByFloorNum(pickupFloorNum);
    int diffFloor[pickupList.size()];
    int positive = 0;
    int negative = 0;
    ...
    for(int i = 0; i < pickupList.size(); i++) {
        ...
        // Condition 2
        if (pickupList.at(i) < 48) {
            return false;
        }
        // Condition 4
        if (pickupList.at(i) - 48 >= floor.getNumPeople()) {
            return false;
        }
        // Condition 5 pt1
        Person person = floor.getPersonByIndex(pickupList.at(i) - 48);
        diffFloor[i] = person.getTargetFloor() - pickupFloorNum;
